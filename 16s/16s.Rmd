...
#    binary_fun, col, binary_fun, UpSet.Ori, df.direct.family)

```

```{r Venn, warning=FALSE, message=FALSE}
# Extract the rows where the value is 1 for each column
Cont <- rownames(df.direct.family)[df.direct.family$Control == 1]
Plun <- rownames(df.direct.family)[df.direct.family$Plunge == 1]
Rate <- rownames(df.direct.family)[df.direct.family$Rate == 1]
...
# rm(B, B2, sample_data, 
#    otu_table, abundance, D, E,
#    binary_fun, col, binary_fun, upset.Rich, df.enriched.family)
```
...
#### Direct treatment
```{r TOP10.Direct, warning=FALSE, message=FALSE}
##### create a function
...
### Plot the graph for Gemmataceae , Direct
```{r TOP10.Direct.Bacillaceae, warning=FALSE, message=FALSE}
...
```
### Plot the graph for Bryobacteraceae, Direct
```{r TOP10.Direct.Bryobacteraceae, warning=FALSE, message=FALSE}
...
# Clean up by removing objects that are no longer needed
rm(physeq.a.genus, meta, otudf, AyBCode.percent, stat.test1, Sporomusaceae.Rich, stats_list)
```
...
```{r Venn, warning=FALSE, message=FALSE}
# Extract the rows where the value is 1 for each column
Direct.Control <- rownames(df.Category.family)[df.Category.family$Direct.Control == 1]
Enriched.Control <- rownames(df.Category.family)[df.Category.family$Enriched.Control == 1]

Direct.Plunge <- rownames(df.Category.family)[df.Category.family$Direct.Plunge == 1]
Enriched.Plunge <- rownames(df.Category.family)[df.Category.family$Enriched.Plunge == 1]

Direct.Rate <- rownames(df.Category.family)[df.Category.family$Direct.Rate == 1]
Enriched.Rate <- rownames(df.Category.family)[df.Category.family$Enriched.Rate == 1]

# Create a list with the extracted data
list_data.1 <- list("Direct" = Direct.Control, "Enriched" = Enriched.Control)
list_data.2 <- list("Direct" = Direct.Plunge, "Enriched" = Enriched.Plunge)
list_data.3 <- list("Direct" = Direct.Rate, "Enriched" = Enriched.Rate)
